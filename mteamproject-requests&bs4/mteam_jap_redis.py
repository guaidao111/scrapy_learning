...
r = redis.StrictRedis(host='127.0.0.1', port=6379) # 新建Redis对象
thread_on = True # 初始化线程启动flag
...
def download(i):
    ...
    while thread_on: # 初始化了线程是否启动，当信号改变为false时则结束线程
        link = r.lpop('mteam.queue') # 将link来源设定为rpush到右边的link从左侧（列表的第一个元素）pop出来
        if link:
            data = parse_page_info(link) # 将队列的link传入解析模块，返回data
            process_data(data)  # 调用处理模块
            print('remaining queue: %s' % r.llen('mteam.queue'))  # 显示列表长度
        time.sleep(0.2)
    print('Thread-%s exit now' % i)

def sigint_handler(signum, frame):  # 定义一个信号，设定数字和frame
...
if __name__ == '__main__':
    ...
    for link in reallink:
        if r.sadd('mteam.seen', link):  # sadd将数据存入set集合，保持link不会重复输入
            r.rpush('mteam.queue', link)
        # data = parse_page_info(link)
        # process_data(data)

    # 创建一个多线程函数,并将线程对象放入一个列表保存
    for i in range(threads_num):
        t = threading.Thread(target=download, args=(i+1,)) # args将需要的参数传入target指定的参数
        t.start() # 启动线程
        threads.append(t)
    # 设定一个信号
